...
import DoctorCardWomen from '../components/DoctorCardWomen';
import DoctorSkeletonWomen from '../components/DoctorSkeletonWomen';
import api from '../../../../api/client';
import { setConsultationMode } from '../../redux/slices/BookingSlice';
import { scale, verticalScale } from '../../../../utils/styling';
import { COLORS, FONT, SIZES } from '../../../../config/constants';
import BackbtnWomen from '../components/BackbtnWomen';
import { hospitalApi } from '../../services/hospital.api';

const WomenDoctorsScreen = () => {
 ...
 const routeMode = route.params?.mode;
  ...
  const fetchCategories = async (isRefresh = false) => {
    try {
      if (!isRefresh) setLoadingCategories(true);

      const res = await hospitalApi.getcategories({ mode: 'BOTH', women: true });
      ...
      if (!isRefresh) setLoadingCategories(false);
    }
  };
    ...
    try {
      ...
      if (routeHospitalId) {
        response = await api.get(
          `/hospital/user/hospital/${routeHospitalId}/doctors`,
          {
            params: {
              mode: finalMode,
              women: true
      ...
      const mappedDoctors = (response?.data?.data || response?.data?.doctors || []).map(item => ({
        id: item.id.toString(),
      ...
      setError('Failed to refresh doctors');
    } finally {
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false}>
        {[
          ...
          >
            <Text>
              {
                mode.value === "OFFLINE" ? < FontAwesome5 name="hospital" size={14} color={finalMode === mode.value ? "#FFFFFF" : "#000"} /> : < Icon name="video-outline" size={18} color={finalMode === mode.value ? "#FFFFFF" : "#000"} />
              }
            </Text>
            <Text
              style={[
  ...
  return (
    <View style={styles.container}>
      <View style={styles.header}>
        <BackbtnWomen onPress={() => navigation.goBack()} />
      ...
      <FlatList
        ...
        renderItem={({ item }) =>
          loading ? <DoctorSkeletonWomen /> : <DoctorCardWomen doctor={item} />
        }
...
export default WomenDoctorsScreen;
